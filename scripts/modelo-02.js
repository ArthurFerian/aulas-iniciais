...
const vendaJogos = [15, 12, 8, 10, 25, 30, 18];

const diasDaSemana = ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sab"]


let maiorVenda = vendaJogos[0];
let diaMaiorVenda = "";
...
for (let i = 0; i < vendaJogos.length; i++) {
    if (vendaJogos[i] > maiorVenda) {
        maiorVenda = vendaJogos[i];
        diaMaiorVenda = diasDaSemana[i];
    }
    totalJogosVendidos +=vendaJogos[i];
...
console.log(`Total de jogo vendidos na semana: ${totalJogosVendidos}`);
